...
class CVAE(object):
    ...
    Attributes:
        ...
        labels (tf placeholder): ph for one-hot labels used as input to model
    ...
    def _define_recognition_network(self):
        ...
        self.weights_enc = []
        self.biases_enc = []
        # concatenate input images and labels so that lvs can take care of non-
        # class variations in dataset
        z_enc = [tf.concat([self.x, self.labels], 1)]
        for layer in range(self.num_layers_enc):
            with tf.variable_scope(str('layer_%01i' % layer)):

                # initialize weights
                if layer == 0:
                    in_size = self.input_size + self.num_classes
                else:
    ...
    def _define_generator_network(self):
        ...
        self.weights_dec = []
        self.biases_dec = []
        # concatenate lvs and labels since lvs (ideally) don't contain
        # information about class identity
        z_dec = [tf.concat([self.z, self.labels], 1)]
        for layer in range(self.num_layers_dec):
            with tf.variable_scope(str('layer_%01i' % layer)):

                # initialize weights
                if layer == 0:
                    in_size = self.num_lvs + self.num_classes
                else:
    ...
    def generate(self, sess, z_mean=None, label=None):
        ...
        if label is None:
            pos = int(
                np.floor(np.random.uniform(low=0.0, high=self.num_classes)))
            label = np.zeros(shape=(1, self.num_classes))
            label[:, pos] = 1
    ...
    def recognize(self, sess, x, label):
        ...
        label should be an integer that will be converted into a one-hot 
        representation if not already in that format
        """
